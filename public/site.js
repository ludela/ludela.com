...
function panels ($) {
  ...
  $document.on('setpanel', function (ev, index) {
    if($splash.length) {
      ...
      var $panel = $((".splash__content:eq(" + (index - 1) + ")"));
...
function scroller ($) {
  ...
  if ($panels.length) {
    ...
    $window.on('scroll', function (ev) {
      ...
      var dir = Math.min(1, Math.max(-1, scrollDiff));
      console.log(dir);
      $document.trigger('nextpanel', dir);
    });

    $window.on('load resize', index(function () {
